...
Kafka 消息
  ...
  → T2ExtractionStep：调用 T2.extract_entities 抽取mention和关系，
     ...
     再把关系写 Neo4j（不经过PG中转）
  → T4IndexingStep：调用后端内部 EmbeddingService 生成文本向量，写 Milvus，同时写 Elasticsearch 全文索引
...
### 4.6 向量化的实际定位：后端基础设施能力

当前实现已把文本/图像 embedding 生成从 T-Agent 调用链中移出，沉到后端内部 `EmbeddingService`。也就是说，后端不再通过 `AgentProxyClient.call("T4", "generate_text_embedding" / "generate_image_embedding", ...)` 调算法服务生成向量，而是直接使用 `llm.embedding.*` 配置调用 embedding 服务。检索逻辑（混合检索：ES 全文 + Milvus 向量 + Neo4j 图谱关联，以及文搜图查 `image_embeddings`）仍由后端自己的 `SearchService` 实现。向量维度当前固定为 4096 维（Milvus Collection 建表时定死），embedding 服务返回的向量维度必须与此一致，否则写入失败。
...
2. **图像向量化**：调用后端内部 `EmbeddingService.generateImageEmbedding`，写入 Milvus
...
调用约定：统一 `POST`，`Content-Type: application/json`，`Accept: application/json`，业务失败也返回 HTTP 200（通过响应体字段表达失败，不是用 4xx/5xx）。超时：T1/T2/T3 当前都是结构化 LLM 服务，算法侧部署常见配置为 `LLM_TIMEOUT_SECONDS=360`、`LLM_MAX_RETRIES=2`，单次后端 HTTP 调用必须覆盖算法侧最多 3 次 LLM 尝试，所以 `AgentProxyClient` 对 T1/T2/T3 设置了 1200 秒最小超时下限；也可以在 `sub_agent_registry.timeout_seconds` 里继续调大。T6 因为耗时较长且不适合部分重试造成状态不一致，**只调用一次，不做网络层自动重试**，其余 Agent 有 2 次重试。embedding 生成不走 AgentProxy/T4，而是后端内部 `EmbeddingService` 直接调用 embedding 服务。
...
除了入库线的实时流水线，后端还有 8 个后台定时任务，其中 3 个会调用 T-Agent；图片向量化使用后端内部 `EmbeddingService`，不再作为 T4 Agent 调用：
...
| `ImageEmbeddingJob` | 每10分钟 | 回填图片内容的T1标注 + 图片向量化 | **T1**(annotate) + 后端 `EmbeddingService` |
...
| `EntityDeduplicationJob` | 每1小时 | 实体归一去重（离线全局融合） | **T3**(resolve_batch，离线路径) + 后端 `EmbeddingService`/Milvus 候选召回 |
...
| **Milvus** | 文本向量、图像向量、实体语义向量、账号语义向量（4个Collection，均4096维） | 后端 `EmbeddingService` 生成向量后写；`SearchService` 混合检索读 |
| **MinIO** | 图片/视频等原始媒体文件对象存储 | 入库时写；后端图片向量化时读 |
| **Redis** | 缓存层 | 各处按需使用 |
...
- [ ] 我的 action 名称是否和接口规约文档一致（T1三个：`annotate`/`annotate_account`/`annotate_event_heat`；其余：`extract_entities`、`resolve_batch`、`complete_profile`、`identify_targets`；embedding 生成不再作为 T4 action 交付）
