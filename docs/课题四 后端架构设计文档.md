...
调用约定：统一 `POST`，`Content-Type: application/json`，业务失败也返回 HTTP 200（通过响应体字段表达失败，不是用 4xx/5xx）。超时：T1/T2/T6 默认 30 秒，T3/T5 默认 60 秒（可在 `sub_agent_registry.timeout_seconds` 单独调整）。T6 因为耗时较长且不适合部分重试造成状态不一致，**只调用一次，不做网络层自动重试**，其余 Agent 有 2 次重试。embedding 生成不走 AgentProxy/T4，而是后端内部 `EmbeddingService` 直接调用 embedding 服务。
