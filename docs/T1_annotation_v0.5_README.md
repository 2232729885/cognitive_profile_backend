...
> **修订说明**：本版本已按 `T1_标注属性.md` 讨论稿的最终结论修正——删除了 `entities_hint_stance`（实体/对象级立场，下沉给 T2 做关系抽取）和 `public_attitude`（民众态度，下沉到后续聚合模块），加回 `bend_tactics`（BEND 叙事操纵手法，讨论稿维度10）。`schema_version` 仍沿用 `t1_annotation_v0.5`，不单独升版本号，因为这是对齐讨论稿的修正，不是新一轮设计变更。
...
T1 中的 `entities_hint` 只作为 T1 自动标注过程中的实体线索，不替代 T2 的正式实体抽取、关系抽取或事件抽取结果。
...
当前 `annotations` 共包含 14 个标注维度（对齐 `T1_标注属性.md` 讨论稿最终确认的10个核心维度 + 4个辅助客观维度）：

| 类别 | 维度数量 | 维度 |
|---|---:|---|
| `high_value_subjective` | 8 | `ideology`、`core_stance`、`opinion_emotion`、`event_heat`、`language_style`、`content_purpose`、`risk_level`、`bend_tactics` |
...
| `bend_tactics` | 判断内容是否体现 BEND 叙事操纵手法 |
...
如果一条内容中多个实体表达了不同立场，`core_stance` 可以为 `mixed`。具体某个实体对另一实体的立场判断不再由 T1 承担，下沉给 T2 的正式关系抽取处理。

---

## 16. `bend_tactics`：BEND 叙事操纵手法

> 本节替代原来的 `entities_hint_stance`（第16节）和 `public_attitude`（第17节）。这两个维度按 `T1_标注属性.md`
> 讨论稿第四章的结论删除：`entities_hint_stance`（实体/对象级立场）下沉给 T2 做正式关系抽取；`public_attitude`
> （民众态度）下沉到后续聚合模块，原因是单条内容通常无法代表整体民众/网民态度，需要结合评论、转发、时间分布才能
> 判断，容易把作者态度或单条内容态度误标成群体态度。为保留章节编号连续性，原17节序号不再使用。
...
"bend_tactics": [
  {
    "tactic": "Engage | Explain | Excite | Enhance | Dismiss | Distort | Dismay | Distract",
...
`bend_tactics` 判断内容是否体现 BEND 叙事操纵手法（对齐 `T1_标注属性.md` 讨论稿维度10），为 T6 重点目标识别提供内容级特征。一条内容允许命中多个手法，也允许不命中（返回空数组）。

### 16.3 字段说明

| 字段 | 类型 | 含义 |
|---|---|---|
| `tactic` | enum | 命中的 BEND 手法名称 |
| `confidence` | number \| null | 该条手法判断的置信度 |
| `evidence` | string \| null | 支撑该判断的原文证据片段 |
| `reason` | string \| null | 判定理由 |

### 16.4 `tactic` 枚举值

| 枚举值 | 中文名 | 含义 |
|---|---|---|
| `Engage` | 参与 | 提出与用户有关的重要话题 |
| `Explain` | 解释 | 详细阐述主题 |
| `Excite` | 激发 | 引发喜悦、兴奋等积极情绪 |
| `Enhance` | 增强 | 鼓励继续讨论该话题 |
| `Dismiss` | 否定 | 否定该话题的重要性 |
| `Distort` | 歪曲 | 歪曲话题的主要内涵 |
| `Dismay` | 泄气 | 引发悲伤、愤怒等负面情绪 |
| `Distract` | 分散 | 讨论毫不相关的话题 |

### 16.5 边界说明

`bend_tactics` 只做内容级叙事操纵手法标注，不做以下任务：

- 不做实体/对象级立场标注（已下沉给 T2）；
- 不做民众态度标注（已下沉到后续聚合模块）；
- 不做长期画像判断，不判断账号是否为"操纵者"，只判断这条内容本身是否体现某种手法。
...
"quality_control": {
  ...
  "failed_modules": [
    "text_aigc_detection | image_aigc_detection | video_aigc_detection | multimodal_aigc_detection | ideology | core_stance | bend_tactics | opinion_emotion | event_heat | language_style | content_purpose | risk_level | topic_tags | account_type | entities_hint | keywords | summary | event_type | none | other"
  ]
}
```
...
`entities_hint` 可以被 T2 参考或调用，但不要求 T2 必须依赖 T1 输出。  
...
7. `annotations.high_value_subjective.opinion_emotion`
8. `annotations.high_value_subjective.event_heat`
9. `annotations.basic_objective.topic_tags`
10. `annotations.basic_objective.account_type`
11. `annotations.basic_objective.entities_hint`
12. `annotations.basic_objective.summary`
13. `evidence_clues`
14. `quality_control`
15. `overall_confidence`
16. `processed_at`
